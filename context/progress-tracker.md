...
- Renamed generated-project copy:
  - `Generated by LaunchKit.` -> `Generated by BaseForge.`;
  - `Generated with LaunchKit.` -> `Generated with BaseForge.`;
  - `LaunchKit project` -> `BaseForge project`.
- Renamed non-public fixture/temp names:
  - `launchkit-demo` -> `baseforge-demo`;
  - `launchkit-smoke-*` -> `baseforge-smoke-*`;
  - `launchkit_smoke` -> `baseforge_smoke`;
  - `launchkit-cli-*` -> `baseforge-cli-*`;
  - `launchkit-shared` -> `baseforge-shared`;
  - `launchkitSharedPlaceholder` -> `baseforgeSharedPlaceholder`.
- Corrected active planning-doc leftovers:
  - `LaunchKitError` -> `BaseForgeError`;
  - stale `@launchkit/*` namespace note -> `@baseforge/*`.

Intentionally not changed:

- Root repo/package name `launchkit`.
- GitHub URLs pointing at `DavidAsrorxonov/launchkit`.
- Repo-layout examples still showing `launchkit/`.
- Historical note about the unavailable `create-launchkit` npm package.
- Historical records.

Remaining active `launchkit` hits after Step 4:

- Root package name in `package.json` and `package-lock.json`.
- GitHub URLs in `packages/cli/package.json`, landing nav/footer, and
  `context/ui/new-ui.md`.
- Repo-layout examples in `context/architecture.md` and
  `context/project-overview.md`.
- Historical `create-launchkit` package-name availability notes in active
  planning docs.
